...
export default async function versionFinder(folderPath: string, versionStr: string) {
  ...
  const files = msbFiles
    .map(file => {
      ...
      const matchesVersion = isNegatedVersion ? reader.metadata.Build !== version : reader.metadata.Build === version;

      return matchesVersion ? { filePath: file, version: reader.metadata.Build } : null;
    })
